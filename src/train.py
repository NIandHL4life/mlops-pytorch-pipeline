"""
Training script for PyTorch image classification model.
Supports structured JSON logging, config loading, and early stopping.
"""
...
def evaluate(
    ...
    """Evaluates model on validation loader and returns (avg_loss, accuracy)."""
    model.eval()
    total_loss = 0.0
    correct = 0
    total = 0
...
def main():
    # Resolve config path: /app/configs (mounted in container) or configs/ (local repo)
    config_path = Path("/app/configs/training_config.yaml")
    if not config_path.exists():
        config_path = Path("configs/training_config.yaml")

    if not config_path.exists():
        print(json.dumps({"error": f"Config not found at {config_path}"}), file=sys.stderr)
        sys.exit(1)
    ...
    print(json.dumps({"event": "training_started", "epochs": config["training"]["epochs"]}), flush=True)
